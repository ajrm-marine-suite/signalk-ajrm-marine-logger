...
Version `0.6.9` recognises timestamped Signal K subsets and rebatched copies of
Logger's own injected replay updates for 15 seconds. This prevents replay data
that still carries its original `YDEN.*` source from being misreported as live
physical input, while genuinely new sensor deltas remain quarantined during
recomputation. Ordinary sailing capture never enters that quarantine path and
continues to retain YDEN data.
...
```bash
cd ~/.signalk
npm install git+https://github.com/ajrm-marine-suite/signalk-ajrm-marine-logger.git#v0.6.9 --omit=dev --no-package-lock
...
- Disable or disconnect live sensor inputs for a valid recomputation test.
  ...
  live inputs from influencing other plugins before their outputs are
  captured. Explicitly configured physical source IDs remain part of this
  isolation check even if they did not occur in the parent recording. This
  isolation applies only to an armed recomputed-result capture; normal sailing
  capture does not suppress or quarantine YDEN data.
